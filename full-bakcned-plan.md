...
| Phase 5 | Provider integrations | In progress (RateHawk HP-1 + HP-2 hotel done) | Hotels are now provider-backed end to end in the package option flow; flights/transfers/activities confirmed NOT available via RateHawk API and stay manual/enquiry-only | HP-1 foundation: `src/server/providers/ratehawk/{config,client,hotels,errors}.ts` + protected `GET /api/admin/providers/ratehawk/hotel-search-test` (production client: Basic auth, timeouts, retry/backoff, circuit breaker, concurrency limiter, rate-limit capture, sanitized logs). HP-2 wiring: live RateHawk hotel rates flow through the existing public options API (`/api/public/trips/[destination]/[trip]/options?type=hotel`) for segments whose `metadata.hotel_source` is `live`/`hybrid` with a `ratehawk.region_id`; manual `trip_hotel_options` remain as fallback. Price fix verified (reads `payment_options.payment_types[].show_amount`). Verified endpoints only: `overview`, `search/multicomplete`, `search/serp/region`, `hotel/info`. Frontend `TripItineraryPlanner` shows a "Live rates" badge + existing loading/error states. HP-1 smoke `7/7` (`ratehawk-hotel-foundation-latest.md`); HP-2 smoke `10/10` (`ratehawk-hp2-latest.md`) — 12 live Armenia hotels via public API, no hash/secret leak, live option selected, booking attached the option session, unknown option rejected, manual fallback preserved. No migration needed (mapping on segment metadata + existing `external_providers`/`provider_quote_snapshots`). HP-3 prebook, HP-4 booking, HP-5 static-content sync, Stripe still pending (see `dev-left/phase-5/left.md`). |
...
- Hotel API integration.
  ...
  - HP-3 prebook/recheck: pending. Server-only `search_hash`/`match_hash` are already captured in the quote snapshot for this step. Candidate paths: hotelpage `POST /api/b2b/v3/search/hp/`, prebook `POST /api/b2b/v3/hotel/prebook/` (both in the key's overview permission list).
  - HP-4 checkout/booking with prebook gate: pending.
